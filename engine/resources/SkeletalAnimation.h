...
namespace engine
{
		...
		std::size_t boneCount() const
		{
			...
			for (const auto& node : nodes)
			{
				if (node.isBone && node.boneIndex >= 0)
				{
					count = std::max(
						count,
						static_cast<std::size_t>(node.boneIndex + 1)
					);
				}
			}
	...
	inline std::size_t findPositionKey(
	...
	inline std::size_t findRotationKey(
	...
	inline std::size_t findScaleKey(
	...
	inline glm::vec3 interpolatePosition(
		const std::vector<KeyPosition>& keys,
		float time)
	{
		if (keys.empty())
	...
	inline glm::quat interpolateRotation(
		const std::vector<KeyRotation>& keys,
		float time)
	{
		if (keys.empty())
	...
	inline glm::vec3 interpolateScale(
		const std::vector<KeyScale>& keys,
		float time)
	{
		if (keys.empty())
	...
	inline glm::quat extractRotation(const glm::mat4& matrix)
	{
		glm::vec3 scale = extractScale(matrix);

		glm::mat3 rotationMatrix(
			scale.x != 0.0f ? glm::vec3(matrix[0]) / scale.x : glm::vec3(1.0f, 0.0f, 0.0f),
			scale.y != 0.0f ? glm::vec3(matrix[1]) / scale.y : glm::vec3(0.0f, 1.0f, 0.0f),
			scale.z != 0.0f ? glm::vec3(matrix[2]) / scale.z : glm::vec3(0.0f, 0.0f, 1.0f)
		);

		return glm::normalize(glm::quat_cast(rotationMatrix));
	}

	inline glm::mat4 evaluateTrackWithBindFallback(
		const SkeletonNode& node,
		const BoneTrack& track,
		float time)
	{
		glm::vec3 bindPosition = extractTranslation(node.bindLocalTransform);
		glm::quat bindRotation = extractRotation(node.bindLocalTransform);
		glm::vec3 bindScale = extractScale(node.bindLocalTransform);

		glm::vec3 position = track.positions.empty()
			? bindPosition
			: interpolatePosition(track.positions, time);

		glm::quat rotation = track.rotations.empty()
			? bindRotation
			: interpolateRotation(track.rotations, time);

		glm::vec3 scale = track.scales.empty()
			? bindScale
		...
		int debugDepth = 0)
	{
		...
		if (clip)
		{
			...
			{
				localTransform = evaluateTrackWithBindFallback(node, *track, time);
				hasTrack = true;
			}
		}
		...
		if (node.isBone &&
			node.boneIndex >= 0 &&
			node.boneIndex < static_cast<int>(outBoneMatrices.size()))
		{
			glm::mat4 finalMatrix =
				skeleton.globalInverseTransform *
				globalTransform *
				node.inverseBindMatrix;

			outBoneMatrices[node.boneIndex] = finalMatrix;

			if (debugFirstFrame && debugDepth < 3)
			{
				std::cout
					<< "  [Pose] Bone " << node.boneIndex
					<< " (" << node.name << "):\n"
					<< "    Local[3]: ["
					<< localTransform[3][0] << ", "
					<< localTransform[3][1] << ", "
		...
		std::vector<glm::mat4>& outGlobalTransforms)
	{
		...
		if (node.isBone &&
			node.boneIndex >= 0 &&
			node.boneIndex < static_cast<int>(outGlobalTransforms.size()))
		{
